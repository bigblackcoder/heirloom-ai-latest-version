...
export default function ActivityFeed({ today, yesterday, older, isLoading }: ActivityProps) {
  ...
  if (!today?.length && !yesterday?.length && !older?.length) {
    return (
    ...
    return (
      ...
      >
        ...
        <div className="flex-1">
          <p className={`font-medium ${isMobile ? 'text-sm' : 'text-base'}`}>{activity.title}</p>
