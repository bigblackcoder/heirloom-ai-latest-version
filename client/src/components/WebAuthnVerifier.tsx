...
const WebAuthnVerifier: React.FC = () => {
  const [status, setStatus] = useState<
    "idle" | "registering" | "authenticating" | "success" | "error"
  >("idle");
  ...
  const startRegistration = async () => {
    try {
      ...
      options.publicKey.user.id = Uint8Array.from(
        atob(options.publicKey.user.id),
        (c) => c.charCodeAt(0),
      );
  ...
  const startHybridRegistration = async () => {
    try {
      ...
      setMessage(
        `${getBiometricName()} verified! Now capturing face for server-side recognition...`,
      );
      setAuthProgress(50);
      ...
      setStatus("success");
      setMessage(
        `Two-factor registration complete! Your device biometrics and facial features are now registered.`,
      );
      ...
      if (errorMessage.includes("Permission denied")) {
        errorMessage =
          "Camera access was denied. Please allow camera access for facial verification.";
      } else if (errorMessage.includes("aborted")) {
        errorMessage = `Registration was cancelled`;
      } else if (errorMessage.includes("NotAllowedError")) {
        errorMessage =
          "Permission to use camera or biometric sensor was denied";
      }
  ...
  const startAuthentication = async () => {
    try {
      ...
      if (isHybrid) {
        ...
        setMessage(
          `${getBiometricName()} verified! Now capturing face for additional security...`,
        );
        ...
        try {
          ...
          throw new Error(
            "Camera access is required for two-factor authentication",
          );
        }
      }
      ...
      if (isHybrid) {
        setMessage(
          `Two-factor authentication successful! Both your device biometrics and facial features were verified.`,
        );
      } else {
        ...
        {!isWebAuthnSupported && (
          <Alert variant="destructive">
            ...
            <div>
              ...
              <AlertDescription>
                Your browser doesn't support biometric authentication. Please
        ...
        {isWebAuthnSupported && !isPlatformAuthenticatorAvailable && (
          <Alert>
            ...
            <div>
              ...
              <AlertDescription>
                Your device doesn't appear to have {getBiometricName()}{" "}
                configured. You can still use a security key if available.
              </AlertDescription>
            </div>
          </Alert>
        )}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <label
                ...
                className="font-medium text-sm flex items-center gap-1 cursor-pointer"
              >
                ...
                <div className="relative group">
                  ...
                  <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 p-2 w-64 bg-black text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity z-50">
                    Combines device biometrics ({getBiometricName()}) with
                    server-side facial recognition for enhanced security.
                  </div>
                </div>
              </label>
