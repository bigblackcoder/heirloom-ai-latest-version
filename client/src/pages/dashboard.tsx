...
export default function Dashboard() {
  ...
  const getStats = () => {
    return {
      llms: connections ? connections.filter((c: Connection) => c.isActive).length : 3,
      agents: connections ? Math.floor(connections.length / 2) : 7,
      verifiedAssets: 5 // Hardcoded for demo
    };
  };
  ...
  return (
    <ProtectedRoute>
      <div className="min-h-screen bg-[#f5f5f7] pb-20">
          ...
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-lg font-semibold text-gray-900">
                {getGreeting()}, {displayUser?.firstName || 'Leslie'}
              </h1>
        ...
        {/* Identity Capsule Card - Using the green theme from screenshots */}
          ...
          <div className="rounded-xl overflow-hidden bg-[#1a472a] text-white p-4 shadow-md relative">
            ...
            <div className="flex items-center mb-3">
              <div className="w-12 h-12 bg-gray-200 rounded-full overflow-hidden mr-3">
                {displayUser?.avatar ? (
                  <img src={displayUser.avatar} alt="User avatar" className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full bg-[#8ccc5c] flex items-center justify-center text-[#1a472a] font-bold text-lg">
                    {displayUser?.firstName?.[0] || 'L'}
                  </div>
                )}
              </div>
              <div>
                <div className="font-bold text-lg">
                  {displayUser?.firstName || 'Leslie'} {displayUser?.lastName || 'Alexander'}
                </div>
            ...
            <div className="flex justify-between mb-3">
              <div>
                ...
                <div className="font-medium">{stats.llms} LLMs | {stats.agents} Agents</div>
              </div>
              <div>
                <div className="text-sm text-gray-300">Verified Data:</div>
                <div className="font-medium">{stats.verifiedAssets} Assets</div>
              </div>
            </div>
