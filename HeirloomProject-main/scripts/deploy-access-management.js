...
async function main() {
    ...
    try {
        ...
        console.log("Deployment transaction hash:", accessManagement.deploymentTransaction().hash);
        console.log("Waiting for deployment confirmation...");
        
        // Wait for deployment with timeout
        const deploymentTimeout = 180000; // 3 minutes
        const deploymentStart = Date.now();
        let accessManagementAddress;
        
        while (Date.now() - deploymentStart < deploymentTimeout) {
            try {
                await accessManagement.waitForDeployment();
                accessManagementAddress = await accessManagement.getAddress();
                console.log("AccessManagement deployed successfully to:", accessManagementAddress);
                
                // Verify contract deployment
                const code = await ethers.provider.getCode(accessManagementAddress);
        ...
        if (!accessManagementAddress) {
            throw new Error("Deployment failed: timeout exceeded");
        }
        
        // Save deployment info
        const deploymentInfo = {
            AccessManagement: accessManagementAddress,
            ...
            transaction: accessManagement.deploymentTransaction().hash,
        ...
        // Save deployment info
        fs.writeFileSync(
            path.join(deploymentPath, `access-management-deployment-${network.name}.json`),
            JSON.stringify(deploymentInfo, null, 2)
        );
        
        console.log("\nDeployment info saved to:", path.join(deploymentPath, `access-management-deployment-${network.name}.json`));
