...
async function main() {
    ...
    try {
        ...
        console.log("Deployment transaction hash:", analyticsModule.deploymentTransaction().hash);
        console.log("Waiting for deployment confirmation...");
        
        // Wait for deployment with timeout
        const deploymentTimeout = 180000; // 3 minutes
        const deploymentStart = Date.now();
        let analyticsModuleAddress;
        
        while (Date.now() - deploymentStart < deploymentTimeout) {
            try {
                await analyticsModule.waitForDeployment();
                analyticsModuleAddress = await analyticsModule.getAddress();
                console.log("AnalyticsModule deployed successfully to:", analyticsModuleAddress);
                
                // Verify contract deployment
                const code = await ethers.provider.getCode(analyticsModuleAddress);
        ...
        if (!analyticsModuleAddress) {
            throw new Error("Deployment failed: timeout exceeded");
        }
        
        // Save deployment info
        const deploymentInfo = {
            AnalyticsModule: analyticsModuleAddress,
            ...
            transaction: analyticsModule.deploymentTransaction().hash,
        ...
        // Save deployment info
        fs.writeFileSync(
            path.join(deploymentPath, `analytics-module-deployment-${network.name}.json`),
            JSON.stringify(deploymentInfo, null, 2)
        );
        
        console.log("\nDeployment info saved to:", path.join(deploymentPath, `analytics-module-deployment-${network.name}.json`));
        
        console.log("\nDeployment Summary:");
        console.log("-------------------");
        console.log("AnalyticsModule:", analyticsModuleAddress);
