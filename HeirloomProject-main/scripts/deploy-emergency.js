...
async function main() {
    ...
    try {
        ...
        console.log("\nDeploying EmergencyProtocol...");
        const EmergencyProtocol = await ethers.getContractFactory("EmergencyProtocol");
        const emergencyProtocol = await EmergencyProtocol.deploy(
        ...
        console.log("Deployment transaction hash:", emergencyProtocol.deploymentTransaction().hash);
        console.log("Waiting for deployment confirmation...");
        
        // Wait for deployment with timeout
        const deploymentTimeout = 180000; // 3 minutes
        const deploymentStart = Date.now();
        let emergencyProtocolAddress;
        
        while (Date.now() - deploymentStart < deploymentTimeout) {
            try {
                await emergencyProtocol.waitForDeployment();
                emergencyProtocolAddress = await emergencyProtocol.getAddress();
                console.log("EmergencyProtocol deployed successfully to:", emergencyProtocolAddress);
                
                // Verify contract deployment
                const code = await ethers.provider.getCode(emergencyProtocolAddress);
        ...
        if (!emergencyProtocolAddress) {
            throw new Error("Deployment failed: timeout exceeded");
        }
        
        // Save deployment info
        const deploymentInfo = {
            EmergencyProtocol: emergencyProtocolAddress,
            ...
            transaction: emergencyProtocol.deploymentTransaction().hash,
        ...
        // Save deployment info
        fs.writeFileSync(
            path.join(deploymentPath, `emergency-protocol-deployment-${network.name}.json`),
            JSON.stringify(deploymentInfo, null, 2)
        );
        
        console.log("\nDeployment info saved to:", path.join(deploymentPath, `emergency-protocol-deployment-${network.name}.json`));
        
        console.log("\nDeployment Summary:");
        console.log("-------------------");
        console.log("EmergencyProtocol:", emergencyProtocolAddress);
