...
async function main() {
    ...
    try {
        ...
        console.log("Deployment transaction hash:", governanceModule.deploymentTransaction().hash);
        console.log("Waiting for deployment confirmation...");
        
        // Wait for deployment with timeout
        const deploymentTimeout = 180000; // 3 minutes
        const deploymentStart = Date.now();
        let governanceModuleAddress;
        
        while (Date.now() - deploymentStart < deploymentTimeout) {
            try {
                await governanceModule.waitForDeployment();
                governanceModuleAddress = await governanceModule.getAddress();
                console.log("GovernanceModule deployed successfully to:", governanceModuleAddress);
                
                // Verify contract deployment
                const code = await ethers.provider.getCode(governanceModuleAddress);
        ...
        const deploymentInfo = {
            ...
            transaction: governanceModule.deploymentTransaction().hash,
        ...
        // Save deployment info
        fs.writeFileSync(
            path.join(deploymentPath, `governance-module-deployment-${network.name}.json`),
            JSON.stringify(deploymentInfo, null, 2)
        );
        
        console.log("\nDeployment info saved to:", path.join(deploymentPath, `governance-module-deployment-${network.name}.json`));
