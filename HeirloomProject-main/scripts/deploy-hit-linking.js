...
async function main() {
    ...
    try {
        ...
        // Deploy HITLinking
        console.log("\nDeploying HITLinking...");
        const HITLinking = await ethers.getContractFactory("HITLinking");
        const hitLinking = await HITLinking.deploy({
            gasLimit: 5000000,
            maxFeePerGas: ethers.parseUnits("120", "gwei"),
            maxPriorityFeePerGas: ethers.parseUnits("96", "gwei")
        });
        
        console.log("Deployment transaction hash:", hitLinking.deploymentTransaction().hash);
        console.log("Waiting for deployment confirmation...");
        
        // Wait for deployment with timeout
        const deploymentTimeout = 180000; // 3 minutes
        const deploymentStart = Date.now();
        let hitLinkingAddress;
        
        while (Date.now() - deploymentStart < deploymentTimeout) {
            try {
                await hitLinking.waitForDeployment();
                hitLinkingAddress = await hitLinking.getAddress();
                console.log("HITLinking deployed successfully to:", hitLinkingAddress);
                
                // Verify contract deployment
                const code = await ethers.provider.getCode(hitLinkingAddress);
        ...
        const deploymentInfo = {
            ...
            transaction: hitLinking.deploymentTransaction().hash,
        ...
        // Save deployment info
        fs.writeFileSync(
            path.join(deploymentPath, `hit-linking-deployment-${network.name}.json`),
            JSON.stringify(deploymentInfo, null, 2)
        );
        
        console.log("\nDeployment info saved to:", path.join(deploymentPath, `hit-linking-deployment-${network.name}.json`));
        
        console.log("\nDeployment Summary:");
        console.log("-------------------");
        console.log("HITLinking:", hitLinkingAddress);
